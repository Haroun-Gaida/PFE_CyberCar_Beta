...
static esp_err_t serve_file(httpd_req_t *req, const char *filepath) {
    ...
    else if (strstr(filepath, ".ps1"))  httpd_resp_set_type(req, "text/plain; charset=utf-8");
...
static esp_err_t uri_capture_hccapx_get_handler(httpd_req_t *req) {
    ...
    httpd_resp_set_hdr(req, "Content-Disposition", "attachment; filename=CyberCar_capture.hccapx");
...
static esp_err_t uri_stop_attack_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "Stop-attack requested via HTTP");
    esp_event_post(WEBSERVER_EVENTS, WEBSERVER_EVENT_ATTACK_STOP,
                   NULL, 0, portMAX_DELAY);
    return httpd_resp_send(req, "OK", 2);
}
...
static esp_err_t uri_rf_scan_handler(httpd_req_t *req) {
    static char json_buf[1024];
    nrf24_scanner_to_json(json_buf, sizeof(json_buf));
    httpd_resp_set_type(req, "application/json");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, json_buf);
}
static httpd_uri_t uri_rf_scan = { .uri = "/rf-scan", .method = HTTP_GET, .handler = uri_rf_scan_handler };

static esp_err_t uri_rf_scan_start_handler(httpd_req_t *req) {
    nrf24_scanner_start();
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "{\"ok\":true}");
}
static httpd_uri_t uri_rf_start = { .uri = "/rf-scan/start", .method = HTTP_POST, .handler = uri_rf_scan_start_handler };

static esp_err_t uri_rf_scan_stop_handler(httpd_req_t *req) {
    nrf24_scanner_stop();
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "{\"ok\":true}");
}
static httpd_uri_t uri_rf_stop = { .uri = "/rf-scan/stop", .method = HTTP_POST, .handler = uri_rf_scan_stop_handler };
...
static httpd_uri_t uri_payloads = { .uri = "/payloads/*",   .method = HTTP_GET, .handler = common_get_handler };
...
static httpd_uri_t uri_stop_attack   = { .uri = "/stop-attack",      .method = HTTP_POST, .handler = uri_stop_attack_handler };
...
void webserver_run(void) {
    ...
    httpd_register_uri_handler(server, &uri_rf_scan);
    ...
    httpd_register_uri_handler(server, &uri_payloads);
    ...
    httpd_register_uri_handler(server, &uri_rf_start);
    httpd_register_uri_handler(server, &uri_rf_stop);

    ESP_LOGI(TAG, "Webserver started — %d handlers registered.", 28);
}
