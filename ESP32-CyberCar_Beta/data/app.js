...
window.onload = function () {
    ...
    var savedTheme = localStorage.getItem("CyberCar_theme") || "dark";
    document.documentElement.setAttribute("data-theme", savedTheme);

    attachRippleToAll();

    if (localStorage.getItem("CyberCar_disclaimer_v1") === "accepted") {
...
function dismissDisclaimer() {
    localStorage.setItem("CyberCar_disclaimer_v1", "accepted");
    document.getElementById("disclaimer-overlay").style.display = "none";
    init();
}
...
function toggleTheme() {
    ...
    localStorage.setItem("CyberCar_theme", theme);
}
...
/* ── NRF24 RF Scanner ────────────────────────────── */
var rfPollInterval = null;

function _initRfCanvas() {
    var canvas = document.getElementById('rf-canvas');
    if (!canvas) return;
    // Use the parent container's offsetWidth/offsetHeight for a reliable pixel size
    var parent = canvas.parentElement;
    var w = parent ? parent.offsetWidth : 320;
    var h = parent ? parent.offsetHeight : 100;
    if (w < 10) w = 320;  // fallback if still hidden
    if (h < 10) h = 100;
    canvas.width  = w;
    canvas.height = h;
}
...
function startRfScan() {
    var btn = document.querySelector('button[onclick="startRfScan()"]');
    if(btn) btn.disabled = true;
    _initRfCanvas();
    
    fetch('http://192.168.4.1/rf-scan/start', { method: 'POST' })
        .then(function(res) { return res.json(); })
        .then(function(j) {
            document.getElementById('rf-status-badge').textContent = 'SCANNING';
            document.getElementById('rf-status-badge').style.backgroundColor = 'var(--acc-danger)';
            if(!rfPollInterval) {
                rfPollInterval = setInterval(pollRfData, 800);
            }
        })
        .catch(function(err) {
            if(btn) btn.disabled = false;
            document.getElementById('rf-status-badge').textContent = 'ERROR';
            document.getElementById('rf-status-badge').style.backgroundColor = 'var(--acc-danger)';
        });
}

function stopRfScan() {
    fetch('http://192.168.4.1/rf-scan/stop', { method: 'POST' })
        .then(function() {
            document.getElementById('rf-status-badge').textContent = 'IDLE';
            document.getElementById('rf-status-badge').style.backgroundColor = 'var(--md-outline-var)';
            if(rfPollInterval) {
                clearInterval(rfPollInterval);
                rfPollInterval = null;
            }
            var btn = document.querySelector('button[onclick="startRfScan()"]');
            if(btn) btn.disabled = false;
...
function pollRfData() {
    fetch('http://192.168.4.1/rf-scan')
        .then(function(res) { return res.json(); })
        .then(function(data) {
            // Show scanner state in badge
            if (!data.running) {
                document.getElementById('rf-status-badge').textContent = 'NRF IDLE';
                document.getElementById('rf-status-badge').style.backgroundColor = 'var(--md-outline-var)';
            } else {
                document.getElementById('rf-status-badge').textContent = 'NRF ACTIVE · ' + data.scan_count + ' sweeps';
            }

            // Update quick channels (from Module 0 — NRF24 sentinel)
            var map = data.channel_map;
            var ch1  = data.wifi_ch1  === true;
            var ch6  = data.wifi_ch6  === true;
            var ch11 = data.wifi_ch11 === true;

            document.getElementById('rf-ch1').textContent  = ch1  ? 'BUSY' : 'CLEAR';
            document.getElementById('rf-ch1').style.color  = ch1  ? 'var(--acc-danger)' : 'var(--md-on-surface-var)';
            document.getElementById('rf-ch6').textContent  = ch6  ? 'BUSY' : 'CLEAR';
            document.getElementById('rf-ch6').style.color  = ch6  ? 'var(--acc-danger)' : 'var(--md-on-surface-var)';
            document.getElementById('rf-ch11').textContent = ch11 ? 'BUSY' : 'CLEAR';
            document.getElementById('rf-ch11').style.color = ch11 ? 'var(--acc-danger)' : 'var(--md-on-surface-var)';

            // Jamming alert (Module 1 broadband sweep result)
            var alertBox = document.getElementById('rf-jamming-alert');
            var jamText  = document.getElementById('rf-jamming-text');
            if (data.jamming === true) {
                jamText.textContent = 'High interference detected — ' + data.busy_count + '/126 channels busy!';
                alertBox.style.display = 'block';
            } else {
                alertBox.style.display = 'none';
            }

            // Draw spectrum graph from channel_map (Module 1, 126 channels)
            var canvas = document.getElementById('rf-canvas');
            if (!canvas) return;
            // Re-init canvas if width is 0 (happens when tab was hidden on load)
            if (canvas.width < 10) _initRfCanvas();
            var ctx = canvas.getContext('2d');
            var w   = canvas.width;
            var h   = canvas.height;
            if (!map || !Array.isArray(map) || map.length === 0) return;

            var nCh      = map.length; // 126
            var barWidth = w / nCh;
            ...
            // WiFi channel markers (CH 1=12, CH 6=37, CH 11=62 in NRF channel space)
            var markers = [{nrf: 12, label: 'W1'}, {nrf: 37, label: 'W6'}, {nrf: 62, label: 'W11'}];
            ...
            for (var i = 0; i < nCh; i++) {
                var val    = map[i] ? 1 : 0;
                var bh     = val > 0 ? (h - 4) * 0.85 : 2;
                var x      = i * barWidth;
                // Colour by proximity to WiFi channels
                var distW1  = Math.abs(i - 12);
                var distW6  = Math.abs(i - 37);
                var distW11 = Math.abs(i - 62);
                var nearWifi = (distW1 < 5 || distW6 < 5 || distW11 < 5);
                if (val > 0) {
                    ctx.fillStyle = nearWifi ? 'rgba(255,45,85,0.85)' : 'rgba(0,229,255,0.7)';
                } else {
                    ctx.fillStyle = 'rgba(0,229,255,0.06)';
                }
                ctx.fillRect(x + 0.5, h - bh - 2, Math.max(barWidth - 1, 1), bh);
            }
        })
        .catch(function() { /* silent — ESP may be busy */ });
}
...
function setRunningVisible(v) {
    ...
    var fab = document.getElementById("emergency-stop-fab");
    if (fab) fab.style.display = v ? "flex" : "none";
}
...
function setResultVisible(v) {
    ...
    /* Hide emergency stop FAB when the result is being shown (attack is over) */
    if (v) {
        var fab = document.getElementById("emergency-stop-fab");
...
function showResult(status, attack_type, content_size, content) {
    ...
    /* Re-enable stop controls in case they were disabled mid-stop */
    var stopBtn = document.getElementById("stop-attack-btn");
    if (stopBtn) { stopBtn.disabled = false; stopBtn.innerHTML = "&#9632; Stop Attack"; }
    var fab = document.getElementById("emergency-stop-fab");
    if (fab) { fab.disabled = false; fab.style.display = "none"; }
...
/* ── Stop Attack ─────────────────────────────────── */
/**
 * Gracefully stops the currently running attack via POST /stop-attack.
 * The ESP32 runs the correct per-type teardown and marks the attack FINISHED.
 * The UI polls /status and transitions to the result screen showing "Stopped".
 */
function stopAttack() {
    var btn = document.getElementById("stop-attack-btn");
    ...
    var oReq = new XMLHttpRequest();
    oReq.timeout = 6000;
    oReq.open("POST", "http://192.168.4.1/stop-attack", true);
    oReq.onload = function () {
        /* Give the ESP32 a moment to apply the stop then poll */
        setTimeout(getStatus, 600);
    };
    oReq.onerror = function () {
        /* The management AP may briefly disconnect on some attack types.
         * Re-enable the button so the user can retry or power-cycle. */
        if (btn) { btn.disabled = false; btn.innerHTML = "&#9632; Stop Attack"; }
        if (fab) { fab.disabled = false; }
        showError("Could not reach ESP32. The device may have disconnected temporarily — try again.");
    };
    oReq.ontimeout = function () {
        if (btn) { btn.disabled = false; btn.innerHTML = "&#9632; Stop Attack"; }
        if (fab) { fab.disabled = false; }
        showError("Request timed out. The attack may still be stopping — wait a moment and check status.");
    };
    oReq.send();
}

/**
 * Emergency stop — confirms with user then calls stopAttack().
 * Shown as a persistent FAB whenever an attack is running.
 */
function emergencyStop() {
    if (!confirm("\u26a0 Emergency Stop\n\nThis will immediately halt all running attacks and restore the management AP.\n\nContinue?")) return;
    stopAttack();
}
