...
void attack_update_status(attack_state_t state) {
    ...
    if (state == FINISHED || state == TIMEOUT) {
        ...
        CyberCar_display_set_attack_timeout(0);
    }
}
...
static void attack_request_handler(void *args, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    CyberCar_display_set_attack_timeout(attack_config.timeout > 0 ? attack_config.timeout : 0);
...
/**
 * @brief Handles an explicit stop-attack request from the web UI.
 *        Runs the same per-type cleanup as the timeout handler, then
 *        marks the attack FINISHED so the UI can transition away cleanly.
 */
static void attack_stop_handler(void *args, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    if (attack_status.state != RUNNING) {
        ESP_LOGW(TAG, "Stop requested but no attack is running (state=%d)", attack_status.state);
        return;
    }

    ESP_LOGI(TAG, "Attack manually stopped (type=%d)", attack_status.type);

    /* Stop the timeout timer if it is still ticking */
    if (esp_timer_is_active(attack_timeout_handle)) {
        esp_timer_stop(attack_timeout_handle);
    }
    ...
    switch (attack_status.type) {
        ...
        case ATTACK_TYPE_EVIL_TWIN:
            ...
            break;
        case ATTACK_TYPE_BT_SPAM:
            ESP_LOGI(TAG, "Stopping BT SPAM attack...");
            attack_bt_spam_stop();
            break;
        case ATTACK_TYPE_CLONE:
            attack_method_super_clone_stop();
        ...
        case ATTACK_TYPE_BT_PAYLOAD:
            ...
            break;
        default:
            ESP_LOGE(TAG, "Unknown attack type during stop. Cleanup may be incomplete.");
    }

    /* Mark finished and clear display */
    attack_update_status(FINISHED);
    CyberCar_display_set_attack_timeout(0);

    /* Free any accumulated result content so the UI gets a clean stop message */
    if (attack_status.content) {
        free(attack_status.content);
        attack_status.content = NULL;
    }
...
void attack_init(){
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(WEBSERVER_EVENTS, WEBSERVER_EVENT_ATTACK_STOP,    &attack_stop_handler,    NULL));
}
